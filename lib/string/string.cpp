...
namespace rgl {
    ...
    string::string(const string_view& sv) : len(sv.size()) {
        if (len > 0) {
    ...
    string::string(const char* str) : len(str ? strlen(str) : 0) {
        if (len > 0) {
            buffer = new char[len + 1];
    ...
    string::string(size_t count, char ch) : len(count) {
        if (len > 0) {
            buffer = new char[len + 1];
    ...
    string::string(string&& other) noexcept : buffer(other.buffer), len(other.len) {
    ...
    void string::erase(size_t index, size_t count) {
        if (index >= len) return;
        if (count == size_t(-1) || index + count > len) {
            count = len - index;
        }
        if (count == 0) return;
        ...
        if (new_len > 0) {
            new_buffer = new char[new_len + 1];
            if (index > 0) {
                memcpy(new_buffer, buffer, index);
            }
            if (index + count < len) {
                memcpy(new_buffer + index, buffer + index + count, len - (index + count));
            }
            new_buffer[new_len] = '\0';
        }
        delete[] buffer;
    ...
    size_t string::find_first_not_of(const char* chars, size_t pos) const {
        if (!chars) return pos < len ? pos : npos;
        for (size_t i = pos; i < len; ++i) {
            bool found = false;
            for (const char* c = chars; *c; ++c) {
                if (buffer[i] == *c) {
    ...
    size_t string::find_last_not_of(const char* chars, size_t pos) const {
        if (len == 0) return npos;
        size_t start = (pos == size_t(-1) || pos >= len) ? len - 1 : pos;
        for (size_t i = start; i != size_t(-1); --i) {
            bool found = false;
            for (const char* c = chars; *c; ++c) {
                if (buffer[i] == *c) {
